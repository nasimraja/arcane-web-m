...
const Createpresale =  () => { 
	...
	const getApproval = async (_t) => {
		let _web3 = new Web3(web3Provider);
		let _tokenContract = new _web3.eth.Contract(TOKEN_ABI,_t);
		let _allowance = await _tokenContract.methods.allowance(wallet.account,PRIVATE_SALE).call() ;		
	...
	const createPrivatePresale = () => {
		...
		let _softcap = _web3.utils.toWei(softCap.toString()) ;
		let _hardCap = _web3.utils.toWei(hardCap.toString()) ;
		let _saleRate = [_web3.utils.toWei(saleRate.toString()) , tokenTaxAmount*100] ;
		let _vestingScheduled = vestingSchedule;
		let _saleTokenLimit = 0 
		if(onlyTokenHolders){
			_saleTokenLimit = _web3.utils.toWei(minimumToken.toString());
		}
		...
		let _maxAllocation = [_saleTokenLimit , _web3.utils.toWei(maxAllocation.toString())];
